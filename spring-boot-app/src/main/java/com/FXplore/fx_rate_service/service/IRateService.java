...
public interface IRateService {
    ...
    Optional<ExchangeRateResponse> getLatestRate(String pairCode);

    // Return rate history for a pair between two dates
    List<ExchangeRateResponse> getRateHistory(String pairCode, LocalDate from, LocalDate to);
    ...
    Optional<ConversionResponse> convertAmount(BigDecimal amount, String pairCode);

    // Return the official end-of-day fixing for a pair on a given date
    Optional<EodFixingResponse> getEodFixing(String pairCode, LocalDate date);

    // Return the latest rate per pair where the rate is older than 4 hours
    List<ExchangeRateResponse> getStaleRates();

    // Return all active currencies
    List<CurrencyResponse> getAllActiveCurrencies();
}
